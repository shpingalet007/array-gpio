...
constructor(i, pin, o ){
	this._index = i;
	this.pin = pin;
	this.res = rpi.gpio_read_pud(pin);
...
intR (x) {
	if(x === null || x === undefined) {
		this.res = rpi.gpio_enable_pud(this.pin, 0);
	} 
	else if(x === 'pu' || x == 1) {
		this.res = rpi.gpio_enable_pud(this.pin, 2);
	}
	else if(x === 'pd' || x === 0) {
		this.res = rpi.gpio_enable_pud(this.pin, 1);
	}
}

readR() {
	return rpi.gpio_read_pud(this.pin);
}
